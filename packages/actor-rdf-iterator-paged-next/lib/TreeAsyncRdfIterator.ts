...
import { TreeNode } from "@comunica/actor-rdf-metadata-extract-tree";

/**
 * An abstract quad iterator that iterates over several pages.
 */
export abstract class TreeAsyncRdfIterator extends BufferedIterator<RDF.Quad> implements RDF.Stream {

  private readonly startUrl: string;
  private nextUrl: string;
  private node: string;
  protected parentQueque: string[] = [];
   ...
   */
  protected abstract getIterator(url: string, nodeId: string, onNextPage: (nextPage?: string) => void)
  : Promise<RDF.Stream>;
  ...
  protected async startIterator(url: string, node: string): Promise<any> {
    ...
    const it: RDF.Stream = await this.getIterator(url, node, (nextPage?: string) => {
      if (nextPage) {
